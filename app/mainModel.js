...
export class MainModel extends AbstractModel {
  ...
  init() {
    ...
    this.pianoKey1Entity = new AbstractEntity(this.mainImageEntity, 0, 15*8, 7, 8, false, ZXColor.brightRed);
    this.pianoKey1Entity.hide = true;
    this.mainImageEntity.addEntity(this.pianoKey1Entity);
    this.pianoKey2Entity = new AbstractEntity(this.mainImageEntity, 0, 15*8, 7, 8, false, ZXColor.cyan);
    this.pianoKey2Entity.hide = true;
    this.mainImageEntity.addEntity(this.pianoKey2Entity);

    this.enterEntity = new SpriteEntity(this.mainImageEntity, 96, 96, ZXColor.color(0), false, 0, 0);
    ...
    this.willyEntity = new SpriteEntity(this.mainImageEntity, 28*8, 9*8, ZXColor.white, false, 0, 0);
    this.willyEntity.setGraphicsData(this.app.globalData.willy);
    this.mainImageEntity.addEntity(this.willyEntity);

    this.desktopEntity.addEntity(new AbstractEntity(this.desktopEntity, 0, 16*8, 32*8, 8, false, ZXColor.yellow));
    this.airEntity = new AirEntity(this.desktopEntity, 0, 17*8, 32*8, 8, 0.0);
    this.desktopEntity.addEntity(this.airEntity);
    
    this.blackBoxEntity = new AbstractEntity(this.desktopEntity, 0, 18*8, 32*8, 6*8, false, ZXColor.black);
    this.blackBoxEntity.clickColor = '#444444';
    this.desktopEntity.addEntity(this.blackBoxEntity);
    this.slidingTextEntity = new SlidingTextEntity(this.blackBoxEntity, this.app.fonts.zxFonts8x8, 0, 8, 32*8, 8, this.slidingText, ZXColor.yellow, false, {animation: 'toLeft', speed: 15, leftMargin: 256, rightMargin: 256});
    this.blackBoxEntity.addEntity(this.slidingTextEntity);

    this.selectCaveEntity = new SpriteEntity(this.blackBoxEntity, 25, 28, ZXColor.white, false, 0, 0);
    ...
    this.spaceEntity = new SpriteEntity(this.blackBoxEntity, 73, 28, ZXColor.color(3), false, 0, 0);
  ...
  handleEvent(event) {
    ...
    switch (event.id) {
      ...
      case 'pianoKey':
        ...
        var border = attr2&7;
        this.borderEntity.bkColor = ZXColor.color(border);
  ...
  loopModel(timestamp) {
    ...
    this.enterEntity.setPenColor(ZXColor.color(Math.round(steps/6)%2*8+6));
    this.spaceEntity.setPenColor(ZXColor.color(Math.round(steps/6)%2*8+3));
