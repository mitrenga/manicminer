...
export class GameApp extends AbstractApp {
  ...
  constructor(platform, importPath, wsURL) {
    ...
    this.controls = {
      keyboard: this.getControls('keyboard', true),
      mouse: this.getControls('mouse', true),
      touchscreen: this.getControls('touchscreen', true),
      //gamepads: this.getControls('gamepads', true),
      gamepads: this.getControls('develGP', true)
    };
  ...
  getControls(device, getCookie) {
    ...
    if (getCookie) {
      var cfgString = this.getCookie(device, false);
      if (cfgString !== false) {
        try {
